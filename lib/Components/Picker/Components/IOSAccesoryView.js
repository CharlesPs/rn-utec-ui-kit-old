...
class PickerIOSAccesoryView extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            donePressed: false
    ...
    render() {
        ...
        return (<View style={styles.viewContainer}>
                <View style={styles.arrowsContainer}>
                    {!props.onUpPress ? null : (<TouchableOpacity onPress={() => props.onUpPress()}>
                            <View style={[styles.arrow, styles.arrowUp]}/>
                        </TouchableOpacity>)}
                    {!props.onDownPress ? null : (<TouchableOpacity onPress={() => props.onDownPress()}>
                            <View style={[styles.arrow, styles.arrowDown]}/>
                        </TouchableOpacity>)}
                </View>
                <TouchableOpacity 
        // activeOpacity={1}
        onPress={() => props.onDonePress()} onPressIn={() => {
